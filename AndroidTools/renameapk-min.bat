...
set apkname=
set apkver=
IF NOT "%~1"=="" (IF "%~2"=="" ("%~dp0aapt" dump badging "%~1">nul
if ERRORLEVEL 1 (echo ERROR: Badly formatted apk (is it an apk?^)
pause
goto :eof)
for /f "tokens=1,2,5,6 usebackq delims=^'" %%i in (`""%~dp0aapt^" dump badging "%~1"`) do (if "%%i"=="application: label=" (set apkname=%%j)
...
@echo off
echo Success: !apkname!_!apkver!.apk) else (goto domulti)) else (echo.
echo.  %~n0%
echo.  ==============
echo.
